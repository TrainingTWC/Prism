# Fix AM Operations Dashboard - Google Apps Script Setup

## Problem
The AM Operations Dashboard shows "No Results Found" even though data is being logged to Google Sheets because the Google Apps Script endpoint is not properly configured to read and return the data.

## Solution Steps

### Step 1: Update Google Apps Script for AM Operations

1. **Open your Google Sheets document** that contains the "AM Ops Checklist" sheet
2. **Go to Extensions > Apps Script**
3. **Replace the entire script** with the following code:
...
function getData() {
  try {
    ...
    var submissions = dataRows.map(function(row) {
      ...
      // Calculate percentage score if overall score exists
...
5. **Click "Deploy"**
6. **Copy the new Web app URL** (should be the same as before: AKfycbyjqLEJNcV3uS04_unMXdM4_2KBUShANCRrwYY9LqqegKNb46Hr2JohCwHIhzNbZSE)

### Step 3: Test the Fix

1. **Open the dashboard app** at http://localhost:3003
2. **Select "Operations Checklist Dashboard"**
3. **Click "Refresh" button**
4. **You should now see your AM Operations data**

## What This Fixes

1. **Adds `getData()` function** to read from "AM Ops Checklist" sheet
2. **Properly formats data** for the dashboard
3. **Handles empty sheets** gracefully
4. **Calculates percentage scores** correctly
5. **Returns data in expected JSON format**

## Testing

You can also test the endpoint directly by opening this URL in your browser:
```
https://script.google.com/macros/s/AKfycbyjqLEJNcV3uS04_unMXdM4_2KBUShANCRrwYY9LqqegKNb46Hr2JohCwHIhzNbZSE/exec?action=getData
```

It should return JSON data with your AM Operations submissions.