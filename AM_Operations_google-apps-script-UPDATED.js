/**
 * AM OPERATIONS CHECKLIST - GOOGLE APPS SCRIPT (UPDATED VERSION)
...
function doPost(e) {
  try {
    ...
    var header = [
      ...
      'HRBP ID', 'Regional HR ID', 'HR Head ID', 'LMS Head ID', 'Regional Training Manager',
    ...
    var row = [
      ...
      params.regionalTrainingManager || '',
...
function getAMOperationsData() {
  try {
    ...
    var jsonData = rows.map(function(row) {
      ...
      obj.region = row[10] || '';
      obj.hrbpId = row[11] || '';
      obj.regionalHrId = row[12] || '';
      obj.hrHeadId = row[13] || '';
      obj.lmsHeadId = row[14] || '';
      obj.regionalTrainingManager = row[15] || '';
      
      var colIndex = 16; // Starting after metadata columns
