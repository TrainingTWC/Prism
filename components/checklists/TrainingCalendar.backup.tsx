...
const TrainingCalendar: React.FC<TrainingCalendarProps> = ({ trainerId, trainerName }) => {
    ...
    return (
                ...
                <div className="mb-3 sm:mb-4">
                    <div className="flex items-center justify-between mb-3 sm:mb-4">
                        ...
                        <div className="text-sm text-gray-600 dark:text-slate-400">
                            <span className="font-medium text-gray-900 dark:text-slate-200">{actualTrainerName}</span>
                        </div>
                    </div>
        ...
        {/* Right Side - Event Entry Form */}
        <div className="w-96 bg-white dark:bg-gradient-to-br dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 rounded-3xl shadow-xl dark:shadow-2xl flex flex-col p-6 border border-gray-200 dark:border-transparent overflow-hidden">
            <div className="flex items-center justify-between mb-4">
                <div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-slate-100 flex items-center gap-2">
                        <CalendarIcon className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                        {editingEvent ? 'Edit Event' : selectedDate ? 'Add Event' : 'Select Date'}
                    </h3>
                    {selectedDate && (
                ...
                {selectedDate && (
                    <button 
                        onClick={() => { setSelectedDate(null); setEditingEvent(null); resetForm(); }}
                        className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-slate-700/50 hover:bg-gray-200 dark:hover:bg-slate-700 flex items-center justify-center transition-all"
                    >
            ...
            {selectedDate ? (
                <div className="space-y-4 flex-1 overflow-y-auto">
                    {/* Event Type */}
                    <div>
                        <label className="block text-sm font-semibold text-gray-900 dark:text-slate-200 mb-2">Event Type</label>
                        <div className="grid grid-cols-3 gap-2">
                            <button
                                onClick={() => setEventType('store')}
                                className={`py-2 px-3 rounded-xl border-2 flex flex-col items-center justify-center gap-1 transition-all ${eventType === 'store'
                            ...
                            <button
                                ...
                                className={`py-2 px-3 rounded-xl border-2 flex flex-col items-center justify-center gap-1 transition-all ${eventType === 'campus'
                            ...
                            <button
                                ...
                                className={`py-2 px-3 rounded-xl border-2 flex flex-col items-center justify-center gap-1 transition-all ${eventType === 'outdoor'
                            ...
                            {eventType === 'store' && (
                                <div>
                                    <label className="block text-sm font-semibold text-gray-900 dark:text-slate-200 mb-2">Select Store</label>
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full p-2.5 border-2 border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none text-sm"
                                        />
                                        <div 
                                            id="store-dropdown"
                                            className="hidden absolute z-10 w-full mt-2 border-2 border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 max-h-40 overflow-y-auto shadow-lg"
                                        >
                                            {filteredStores.length === 0 ? (
                                                <div className="p-3 text-xs text-gray-500 dark:text-slate-400 text-center">
                                                    No stores found
                                                </div>
                                            ) : (
                                                filteredStores.map(store => (
                                                    <div
                                                        ...
                                                        className={`p-2.5 cursor-pointer hover:bg-purple-50 dark:hover:bg-slate-600 transition-colors text-xs border-b border-gray-100 dark:border-slate-600 last:border-0 ${
                            ...
                            {eventType === 'campus' && (
                                <div>
                                    <label className="block text-sm font-semibold text-gray-900 dark:text-slate-200 mb-2">Select Campus</label>
                                    <div className="relative">
                                        <input
                                            type="text"
                                            ...
                                            className="w-full p-2.5 border-2 border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-sm"
                                        />
                                        <div 
                                            id="campus-dropdown"
                                            className="hidden absolute z-10 w-full mt-2 border-2 border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 max-h-40 overflow-y-auto shadow-lg"
                                            ...
                                            {CAMPUS_LIST.filter(campus => 
                                                ...
                                                CAMPUS_LIST.filter(campus => 
                                                    ...
                                                    <div
                                                        ...
                                                        className={`p-2.5 cursor-pointer hover:bg-green-50 dark:hover:bg-slate-600 transition-colors text-xs border-b border-gray-100 dark:border-slate-600 last:border-0 ${
                            ...
                            {/* Task Type Selection */}
                            <div>
                                <label className="block text-sm font-semibold text-gray-900 dark:text-slate-200 mb-2">Task Type</label>
                                <select
                                    value={selectedTask}
                                    onChange={(e) => setSelectedTask(e.target.value)}
                                    className="w-full p-2.5 border-2 border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none text-sm font-medium"
                                >
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder={selectedTask ? "Add any additional notes..." : "Enter task details..."}
                                    className="w-full p-3 border-2 border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none min-h-[100px] resize-none text-sm"
                                />
                            </div>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button
                                    ...
                                    className="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 rounded-xl shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {editingEvent ? 'Update' : 'Add Event'}
                                </button>
                                {editingEvent && (
                                    <button
                                        onClick={() => { setEditingEvent(null); resetForm(); }}
                                        className="px-4 py-2.5 text-sm font-semibold text-gray-700 dark:text-slate-300 bg-white dark:bg-slate-700 hover:bg-gray-100 dark:hover:bg-slate-600 rounded-xl transition-all border-2 border-gray-200 dark:border-slate-600"
                                    >
                                        Cancel
                                    </button>
                                )}
                        ...
                        <div className="flex-1 flex items-center justify-center text-center p-8">
                            <div>
                                <CalendarIcon className="w-16 h-16 text-gray-300 dark:text-slate-600 mx-auto mb-4" />
                    ...
                    <div className="pt-4 border-t border-gray-200 dark:border-slate-700">
                        <button
                            ...
                            className="w-full px-5 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl font-semibold text-sm transition-all shadow-lg hover:shadow-xl"
                        >
                            {isSubmitting ? 'Submitting...' : `Submit Calendar (${Object.keys(events).length} days)`}
                        </button>
                    </div>
                </div>
            </div>
