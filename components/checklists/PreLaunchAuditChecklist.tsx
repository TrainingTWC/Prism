...
interface PreLaunchAuditChecklistProps {
  userRole: UserRole;
...
const PreLaunchAuditChecklist: React.FC<PreLaunchAuditChecklistProps> = ({ userRole, onStatsUpdate, editMode = false, existingSubmission }) => {
  const { config, loading: configLoading } = useConfig();
  const sections = PRE_LAUNCH_SECTIONS;
  ...
  const [responses, setResponses] = useState<SurveyResponse>(() => {
    ...
    try {
      return JSON.parse(localStorage.getItem('pre_launch_audit_resp') || '{}');
    } catch (e) {
      return {};
    }
  });
  ...
  const [meta, setMeta] = useState<SurveyMeta>(() => {
    ...
    let stored: any = {};
    try {
      stored = JSON.parse(localStorage.getItem('pre_launch_audit_meta') || '{}');
    } catch (e) { }
  ...
  const loadDraftsFromSheet = async () => {
    if (!PRE_LAUNCH_ENDPOINT || !meta.auditorId) return;

    setDraftsLoading(true);
    try {
      const url = `${PRE_LAUNCH_ENDPOINT}?action=getPreLaunchAuditDrafts&auditorId=${encodeURIComponent(meta.auditorId)}`;
      ...
      console.error('Could not load pre-launch audit drafts:', error);
      setDrafts([]);
    } finally {
  ...
  useEffect(() => { localStorage.setItem('pre_launch_audit_resp', JSON.stringify(responses)); }, [responses]);
  useEffect(() => { localStorage.setItem('pre_launch_audit_meta', JSON.stringify(meta)); }, [meta]);
  useEffect(() => {
    try {
      localStorage.setItem('pre_launch_audit_images', JSON.stringify(questionImages));
    } catch (e) {
  ...
  useEffect(() => {
    try { localStorage.setItem('pre_launch_audit_remarks', JSON.stringify(questionRemarks)); } catch (e) { }
  }, [questionRemarks]);
  useEffect(() => { localStorage.setItem('pre_launch_audit_signatures', JSON.stringify(signatures)); }, [signatures]);
  ...
  useEffect(() => {
    ...
    sections.forEach(section => {
      section.items.forEach(item => {
        ...
        if (response === 'na' || !response) return;
        maxScore += item.w;
  ...
  const saveDraft = async () => {
    if (!PRE_LAUNCH_ENDPOINT) { alert('Endpoint not configured.'); return; }
    ...
    try {
      const params = new URLSearchParams({
        action: 'savePreLaunchAuditDraft',
      ...
      const draftResponse = await fetch(PRE_LAUNCH_ENDPOINT, {
        method: 'POST',
  ...
  const loadDraft = async (draftId: string) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${PRE_LAUNCH_ENDPOINT}?action=loadPreLaunchAuditDraft&draftId=${encodeURIComponent(draftId)}`);
      const data = await response.json();
  ...
  const deleteDraft = async (draftId: string) => {
    ...
    setIsLoading(true);
    try {
      const params = new URLSearchParams({ action: 'deletePreLaunchAuditDraft', draftId });
  ...
  const startNewChecklist = () => {
    ...
    localStorage.removeItem('pre_launch_audit_resp');
    localStorage.removeItem('pre_launch_audit_meta');
    localStorage.removeItem('pre_launch_audit_images');
    localStorage.removeItem('pre_launch_audit_remarks');
    localStorage.removeItem('pre_launch_audit_signatures');

    setResponses({});
    setMeta({ auditorName: '', auditorId: '', storeName: '', storeId: '', city: '', region: '' });
    setQuestionImages({});
    setQuestionRemarks({});
  ...
  const autofillForTesting = () => {
    if (confirm('This will fill the ENTIRE pre-launch audit form with test data. Continue?')) {
      setMeta({
        auditorName: 'Test Auditor',
        auditorId: 'AUD001',
        storeName: 'Test Store - Koramangala',
        storeId: 'STR001',
        city: 'Bangalore',
        region: 'South'
      });
      ...
      alert('Pre-launch audit form autofilled with test data! All 40 questions completed. You can now review and submit.');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (missingFields.length > 0) {
      ...
      const infoSection = document.getElementById('pre-launch-audit-information');
      if (infoSection) {
    ...
    try {
      ...
      while (!didSubmit && networkRetryCount <= MAX_NETWORK_RETRIES) {
        ...
        try {
          console.log(`📤 Submitting Pre-Launch Audit (attempt ${networkRetryCount + 1})... Payload: ${(bodyString.length / 1024).toFixed(1)}KB`);

          const response = await fetch(PRE_LAUNCH_ENDPOINT, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: bodyString,
            redirect: 'follow',
          ...
          console.log('📨 Pre-Launch Audit GAS response:', result);

          if (result.success || result.status === 'success') {
            console.log('✅ Pre-Launch Audit submitted successfully');
            didSubmit = true;
          } else {
      ...
      if (!didSubmit) {
        alert('Failed to submit Pre-Launch Audit: ' + lastError + '\nPlease check your internet connection and try again.');
        setIsLoading(false);
        return;
      }
      ...
      console.error('Error submitting pre-launch audit:', error);
        ...
        <div className="max-w-2xl mx-auto text-center">
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-8">
            ...
            <h2 className="text-2xl font-bold text-green-800 dark:text-green-300 mb-2">
              Pre-Launch Audit Submitted Successfully!
            </h2>
            <p className="text-green-700 dark:text-green-400 mb-6">
              Thank you for completing the Pre-Launch Audit. Your responses have been recorded.
            </p>
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 pb-6 sm:pb-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-slate-100 mb-2">
              🚀 Pre-Launch Audit Assessment
            </h1>
            <p className="text-sm sm:text-base text-gray-600 dark:text-slate-400">
              Comprehensive pre-launch readiness audit covering infrastructure, safety, hygiene, and compliance standards for new store openings.
            </p>
          </div>
      ...
      {/* Audit Information */}
      <div id="pre-launch-audit-information" className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={meta.storeName} onChange={(e) => handleMetaChange('storeName', e.target.value)} placeholder="Enter store name"
              className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Store ID</label>
            <input type="text" value={meta.storeId} onChange={(e) => handleMetaChange('storeId', e.target.value)} placeholder="Enter store ID"
              className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100" />
          </div>
        ...
        {activeSection === null ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full bg-gray-200 dark:bg-slate-600 rounded-full h-2 mb-2">
                      <div className={`h-2 rounded-full transition-all duration-300 ${isComplete ? 'bg-green-500' : answered > 0 ? 'bg-indigo-500' : 'bg-gray-300 dark:bg-slate-500'}`}
                        style={{ width: `${pct}%` }} />
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {answered > 0 && (
                        <span className={`text-xs font-bold ${scorePct >= 80 ? 'text-green-600' : scorePct >= 60 ? 'text-indigo-600' : 'text-red-600'}`}>Score: {scorePct}%</span>
                      )}
                    </div>
                  </button>
                );
          ...
          <div>
            ...
            {sections.filter(s => s.id === activeSection).map((section) => {
              const sectionOptions = section.options || ['compliant', 'partially-compliant', 'not-compliant', 'na'];
              const answered = section.items.filter(item => { const key = `${section.id}_${item.id}`; return responses[key] && responses[key] !== ''; }).length;

              return (
                <div key={section.id}>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base sm:text-lg font-semibold text-indigo-700 dark:text-indigo-300">{section.title}</h3>
                    <span className="text-xs font-medium px-3 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full">{answered}/{section.items.length} answered</span>
                  </div>

                  <div className="space-y-4">
                    {section.items?.map((item, itemIndex) => {
                      const serialNumber = `PL-${itemIndex + 1}`;
                      ...
                      return (
                        <div key={item.id} id={`q_${questionKey}`}
                          ...
                          <div className="flex items-start gap-2 sm:gap-3 mb-3">
                            <span className="inline-flex items-center justify-center min-w-[3rem] px-2 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300 rounded-md text-xs font-bold flex-shrink-0">{serialNumber}</span>
                            ...
                            {sectionOptions.map(option => (
                              <label key={option} className="flex items-center gap-2 sm:gap-2 cursor-pointer p-2 sm:p-0 rounded hover:bg-gray-100 dark:hover:bg-slate-700 sm:hover:bg-transparent transition-colors min-h-[44px] sm:min-h-0">
                                <input type="radio" name={`${section.id}_${item.id}`} value={option}
                                  ...
                                  className="w-5 h-5 sm:w-4 sm:h-4 text-indigo-600 border-gray-300 dark:border-slate-600 focus:ring-indigo-500 flex-shrink-0" />
                                <span className="text-sm sm:text-sm text-gray-700 dark:text-slate-300 font-medium">
                                  {option === 'compliant' ? 'Compliance' : option === 'partially-compliant' ? 'Partial Compliance' : option === 'not-compliant' ? 'Non-Compliance' : 'N/A'}
                                </span>
                              </label>
                            ))}
                          </div>
                          ...
                          <div className="pl-0 sm:pl-9">
                            ...
                            <div className="mt-3">
                              ...
                              <textarea value={questionRemarks[`${section.id}_${item.id}`] || ''}
                                ...
                                className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
                            </div>
                          </div>
                        </div>
                      );
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-4 sm:p-6">
        ...
        <div className="space-y-6 md:space-y-0 md:grid md:grid-cols-2 md:gap-6">
          ...
          <div className="space-y-3">
            <label className="block text-base sm:text-sm font-medium text-gray-700 dark:text-slate-300">Store Manager Signature *</label>
            <div className="border-3 border-gray-400 dark:border-slate-500 rounded-xl bg-white p-1 shadow-inner">
              <canvas ref={smCanvasRef} width={800} height={300}
                onMouseDown={(e) => startDrawing(e, 'sm')} onMouseMove={(e) => draw(e, 'sm')} onMouseUp={() => stopDrawing('sm')} onMouseLeave={() => stopDrawing('sm')}
                onTouchStart={(e) => { e.preventDefault(); startDrawing(e, 'sm'); }} onTouchMove={(e) => { e.preventDefault(); draw(e, 'sm'); }} onTouchEnd={(e) => { e.preventDefault(); stopDrawing('sm'); }}
                className="w-full h-auto touch-none rounded-lg" style={{ touchAction: 'none', cursor: 'crosshair', WebkitTouchCallout: 'none', WebkitUserSelect: 'none', userSelect: 'none' }} />
            </div>
            <button type="button" onClick={() => clearSignature('sm')}
              className="w-full px-4 py-3 sm:py-2 bg-red-500 hover:bg-red-600 active:bg-red-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2 min-h-[48px] sm:min-h-0">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
              Clear Signature
            </button>
...
export default PreLaunchAuditChecklist;
