        ...
        <>
          ...
          <div className="bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg shadow-lg p-6 text-white">
            <div className="flex items-center justify-between">
              ...
              {/* Timer Display */}
              <div className={`flex items-center gap-3 px-6 py-3 bg-white/20 backdrop-blur-sm rounded-lg border-2 ${timeRemaining <= 300 ? 'border-red-400 animate-pulse' :
                ...
                <div>
                  ...
                  <div className={`text-2xl font-bold tracking-wider ${timeRemaining <= 300 ? 'text-red-100' :
                  ...
                  <div className="flex items-center justify-between p-2 bg-gray-50 dark:bg-slate-700 rounded">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-24 h-2 bg-gray-200 dark:bg-slate-600 rounded-full overflow-hidden">
                        <div
                          className={`h-full transition-all duration-300 ${noiseLevel > 50 ? 'bg-red-500' : 'bg-green-500'
                            }`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="relative" ref={campusDropdownRef}>
                  <div
                    className={`w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 flex items-center justify-between ${isLocked ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
                      }`}
                      ...
                      <div className="max-h-48 overflow-y-auto">
                        {filteredCampuses.length > 0 ? (
                          filteredCampuses.map((campus) => (
                            <div
                              key={campus}
                              className={`px-4 py-2 cursor-pointer hover:bg-indigo-50 dark:hover:bg-indigo-900/20 ${campusName === campus ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300' : 'text-gray-900 dark:text-slate-100'
                                }`}
                      ...
                      <div key={question.id} className="border-b border-gray-200 dark:border-slate-700 last:border-0 pb-6 last:pb-0">
                        ...
                        <div className="space-y-2">
                          {Object.entries(question.options).map(([key, option]) => (
                            <label
                              key={key}
                              className={`flex items-start gap-3 p-4 rounded-lg border-2 transition-all ${isLocked
