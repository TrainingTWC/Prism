...
interface QAAuditorPerformanceInfographicProps {
...
const QAAuditorPerformanceInfographic: React.FC<QAAuditorPerformanceInfographicProps> = ({ submissions }) => {
    const performanceData = useMemo(() => {
        if (submissions.length === 0) return { top: null, bottom: null };

        const scoresByAuditor: { [key: string]: { name: string, totalPercent: number, count: number } } = {};

        submissions.forEach(s => {
            if (!scoresByAuditor[s.qaName]) {
                scoresByAuditor[s.qaName] = { name: s.qaName, totalPercent: 0, count: 0 };
            }
            scoresByAuditor[s.qaName].totalPercent += parseFloat(s.scorePercentage) || 0;
            scoresByAuditor[s.qaName].count++;
        });

        const aggregated = Object.values(scoresByAuditor).map(({ name, totalPercent, count }) => ({
            name,
    ...
    return (
        <InfographicCard title="QA Performance by Auditor">
            <div className="flex flex-col h-full">
                <div className="flex-1 flex items-center">
                    <PerformanceStat
                        label="Top Performing QA Auditor"
                ...
                <div className="flex-1 flex items-center">
                    <PerformanceStat
                        label="Lowest Performing QA Auditor"
...
export default QAAuditorPerformanceInfographic;