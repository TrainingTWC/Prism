...
interface QAAMPerformanceInfographicProps {
...
const QAAMPerformanceInfographic: React.FC<QAAMPerformanceInfographicProps> = ({ submissions }) => {
    const performanceData = useMemo(() => {
        if (submissions.length === 0) return { top: null, bottom: null };

        const scoresByAM: { [key: string]: { name: string, totalPercent: number, count: number } } = {};

        submissions.forEach(s => {
            if (!scoresByAM[s.amName]) {
                scoresByAM[s.amName] = { name: s.amName, totalPercent: 0, count: 0 };
            }
            scoresByAM[s.amName].totalPercent += parseFloat(s.scorePercentage) || 0;
            scoresByAM[s.amName].count++;
        });

        const aggregated = Object.values(scoresByAM).map(({ name, totalPercent, count }) => ({
            name,
    ...
    return (
        <InfographicCard title="QA Performance by AM">
            <div className="flex flex-col h-full">
                <div className="flex-1 flex items-center">
                    <PerformanceStat
                        label="Top Performing AM"
...
export default QAAMPerformanceInfographic;