...
// Emulate Survey.availableAreaManagers behavior (derive AMs from AREA_MANAGERS and placeholders)
const areaManagersMatches = [];
if (constantsText) {
  // crude parse: find AREA_MANAGERS array block and extract { name: 'X', id: 'Y' }
  const amBlockMatch = constantsText.match(/export const AREA_MANAGERS:[\s\S]*?= \[(.*?)\];/m);
  if (amBlockMatch) {
    const block = amBlockMatch[1];
    const entryRe = /\{\s*name:\s*'([^']+)'\s*,\s*id:\s*'([^']+)'\s*\}/g;
    let m;
    while ((m = entryRe.exec(block)) !== null) {
      areaManagersMatches.push({ name: m[1], id: m[2] });
    }
  }
}

const presentAMs = areaManagersMatches.filter(am => Array.from(amIds).includes(am.id));
const missingFromAreaManagers = Array.from(amIds).filter(id => !areaManagersMatches.some(am => am.id === id));

console.log('\nDerived Area Managers for HR H3728 (from AREA_MANAGERS):');
presentAMs.forEach(am => console.log('-', am.id, am.name));
if (missingFromAreaManagers.length) console.log('Placeholders needed for IDs:', missingFromAreaManagers);
